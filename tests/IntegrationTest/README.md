...
- **結合テストビルド**: `dotnet publish -p:IntegrationTestBuild=true` で `INTEGRATION_TEST` 定数が定義され、
  ...
  - macOSでもログを固定ディレクトリ(`~/Library/Logs`)ではなくプロファイルディレクトリ配下へ出力する
    (テストドライバとCIのartifactから回収するため)
...
- テストドライバは失敗時にプロファイルディレクトリ配下の `Logs/` を出力する。CIでは `tmp/integration/` を
  artifactとして収集する。
- macOS/Linuxでは自己置換と再起動を本体プロセスの外で行う(単一ファイル発行のため実行中に自身を
  差し替えるとクラッシュする)。この外部ヘルパーの動作は `Logs/kevi-update-helper.log` に記録される。
