...
### EQMonitor API Client (`KyoshinEewViewer.EqMonitorApi`)

The client is generated by NSwag from `src/KyoshinEewViewer.EqMonitorApi/openapi.json`. That spec is **not** the upstream file verbatim — it is normalized by `scripts/update-eqmonitor-openapi.py`, which must be re-run to pick up backend changes. Do not hand-edit the spec, and do not point `OpenApiReference` at the raw upstream file: NSwag 14.6.3 fails or silently mis-generates without the normalization.

- **`servers` must be stripped and `/UseBaseUrl:false` must stay.** Otherwise the server URL is baked into the generated source as the `BaseUrl` default. KEVi ships no endpoint; the user supplies it and it reaches the client only via `HttpClient.BaseAddress`.
- **Enum names need `x-enumNames`.** NSwag's default naming maps the JMA intensity values `!5-` and `5-` both to `_5`, producing CS0102. Values differing only in case (`D` / `d`) collide the same way.
- **OpenAPI 3.1 `anyOf` must be folded into 3.0-style `nullable`.** NSwag cannot resolve `anyOf` and emits empty placeholder classes instead, which silently turns `string?` fields and array-valued query parameters into unusable types.
- **Keep `JsonLibrary` on Newtonsoft (the default).** With `/JsonLibrary:SystemTextJson`, NSwag emits `JsonStringEnumConverter<T>`, which matches C# member names rather than `[EnumMember]` values, and omits the converter entirely on collection-of-enum properties — so `"5-"` and `datasources` both fail to deserialize.

`tests/KyoshinEewViewer.Tests/Services/EqMonitorApiClientTests.cs` guards the query-string and deserialization behavior; run it after regenerating.
